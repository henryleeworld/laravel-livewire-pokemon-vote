...
placeholder(
    <<<'HTML'
                    <div class="container px-4 py-8 mx-auto text-white">
                        <div class="mb-6">
                            <input type="text" class="px-4 py-2 w-full placeholder-gray-400 text-white rounded-lg bg-gray-800/40" placeholder="{{ __('Search Pokémon...') }}">
                        </div>
...
<div class="container px-4 py-8 mx-auto text-white">
    <div class="mb-6">
        <input wire:model.live="search" type="search"
            ...
            placeholder="{{ __('Search by name or Pokédex number...') }}">
    </div>

    <div class="grid gap-4">
        @if ($results->isEmpty())
            <div class="py-8 text-center text-gray-400">
			    {{ __('No Pokémon found matching your search.') }}
            </div>
        @else
            @foreach ($results as $index => $pokemon)
                <div
                    ...
                    <div class="flex-grow">
                        ...
                        <h2 class="text-xl font-semibold capitalize">{{ __($pokemon['name']) }}</h2>
                    </div>
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-sm text-gray-400">
                            {{ $pokemon['upVotes'] . __('W') }} - {{ $pokemon['downVotes'] . __('L') }}
                        </div>
                    </div>
                </div>
            @endforeach
